...
class PredictionOverviewWindow(Gtk.Grid):
    ...
    def create_layout(self):
        ...
        prediction_uuid_text_entry = TextEntryWidget("prediction UUID", None, False)
        prediction_uuid_text_entry.set_size_request(500, -1)
        self.attach(prediction_uuid_text_entry, 0, row, 2, 1)

        row += 1

        common_name_text_entry = TextEntryWidget("Common name", None, False)
        self.attach(common_name_text_entry, 0, row, 2, 1)
        ...
        desc_textview_widget = TextViewWidget(desc_textview)

        self.attach(desc_textview_widget, 0, row, 1, 1)

        if self.prediction is not None:
            prediction_uuid_text_entry.set_entry_value(self.prediction.uuid)
            common_name_text_entry.set_entry_value(self.prediction.commonname)
